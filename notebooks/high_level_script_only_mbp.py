...
from classes.pipeline_mbp import Pipeline_MBP
...
variables = {
   ...
   # 'weight': variables['weight'],
    "static_data": {
        "mortality": variables["static_data"]["mortality"],
        "intime": variables["static_data"]['intime'],
        'first_day_end': variables ["static_data"]['first_day_end'],
        'stay_id': variables['static_data']['stay_id']
    }
}
n_features = count_features(variables)
n_seq, n_static = count_sequential_and_static_features(variables)
...
if isinstance(parameters["dataset_type"], list) and len(parameters["dataset_type"]) > 1:
    ...
    ############################# Single Run #################################################
    print("Single Run")
    pipe = Pipeline_MBP(variables=variables, parameters=parameters, new_data=True)
    pipe.run_experiment()
    # pipe.memorize()
    pipe.explain(model_name="lstm_static", method="plot_shap_heatmap_mean_abs", num_samples=1000)
    #pipe.explain(model_name="multi_channel_lstm_static", feature_to_explain = 'mbp_value', method="plot_single_feature_time_shap", num_samples=1000, feature_type='sequential', feature_idx=0, sample_idx=1)
