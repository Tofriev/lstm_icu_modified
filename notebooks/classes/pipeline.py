    ...
    def visualize_sequences(self):
        if 'tudd' in self.parameters['dataset_type'] and 'mimic' in self.parameters['dataset_type']:
            ...
            for feature_idx in range(num_features):
                ...
                sns.kdeplot(mimic_feature_values[feature_idx], label='MIMIC', fill=True, alpha=0.5)
                sns.kdeplot(tudd_feature_values[feature_idx], label='TUDD', fill=True, alpha=0.5)
