...
#train_seq, test_seq = train_test_split(sequences, test_size=0.2, stratify=labels, random_state=42)
#rint(f'Training sequences: {len(train_seq)}, Test sequences: {len(test_seq)}')
...
#rainer.test(model, datamodule=data_module) # mimic
# %%

def stratified_split(sequences, size):
    labels = [seq[1] for seq in sequences]  # Extract labels (mortality)
    stratified_sequences, _ = train_test_split(
        sequences, test_size=(1 - size), stratify=labels, random_state=42
    )
    return stratified_sequences

half_mimic_train = stratified_split(train_seq, size=0.5)
half_tudd_train = stratified_split(tudd_train_seq, size=0.5)

combined_train_seq = half_mimic_train + half_tudd_train

combined_test_seq = test_seq

def label_distribution(sequences, dataset_name):
    labels = [seq[1] for seq in sequences]
    positive_ratio = sum(labels) / len(labels)
    print(f"Positive label ratio in {dataset_name}: {positive_ratio:.2f}")

label_distribution(half_mimic_train, "Half MIMIC Train")
label_distribution(half_tudd_train, "Half TUDD Train")
label_distribution(combined_test_seq, "TUDD Test")

combined_data_module = IcuDataModule(combined_train_seq, combined_test_seq, batch_size=BATCH_SIZE)

trainer.fit(model, combined_data_module)
trainer.test(model, datamodule=combined_data_module)
# %%
